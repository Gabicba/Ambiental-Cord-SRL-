...
- **CSS Variables**: Sistema de tokens OKLCH para temas claro/oscuro (dark mode con clase `.dark`)
- **Paleta de colores**:
  - **Violeta Principal**: `#6C4CFF` — botones primarios, activos, branding
  - **Turquesa Secundario**: `#00C2AB` — badges, iconos, énfasis visual
  - **Verde**: `#22C55E` — estados completados, correctos, éxito
  - **Naranja**: `#F59E0B` — pendientes, advertencias, alertas
  - **Rojo**: `#EF4444` — errores, demorados, críticos
  - **Azul**: `#3B82F6` — información, indicadores neutrales
  - **Fondo claro**: `#F8F7FC` (lavanda muy claro)
  - **Fondo oscuro**: `#0F172A` (slate oscuro)
- **Fuentes**: Poppins (body + headings), sans-serif de respaldo
- **Íconos**: Remix Icon (linear) + Font Awesome 6.4
- **Bordes**: Redondeados generosos (`rounded-xl`, `rounded-2xl`)
- **Dark mode**: Toggle manual con persistencia en localStorage, soporte completo en sidebar, topbar, cards y tablas
